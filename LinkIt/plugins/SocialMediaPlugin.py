...
null_values = ['NA', 'N/A', 'na', 'n/a', 'Na', 'N/a', '', ' ', 'none', 'None', 'NULL', 'null']
social_media_regex = [(re.compile(r"@[A-Za-z0-9_]+"), 100.0)]
...
def get_confidence_score(col_name, col_vals):
    ...
    for elem in col_vals:
        scores.append(get_elem_score(sm_name_check, elem))

    if len(scores) > 0:
        return np.mean(remove_outliers(np.array(scores)))
    else:
        return 0.0

def get_elem_score(sm_name_check, elem):
    for regex, score in social_media_regex:
        if regex.match(elem):
...
def remove_null(col_vals):
    return [val for val in col_vals if val not in null_values]
