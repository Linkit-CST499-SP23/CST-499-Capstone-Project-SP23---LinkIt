...
def get_confidence_score(col_name, col_values):
     ...
     for c in col_values:
        ...
        if format_check and (colcheck_1 or colcheck_2):
            scores.append(100)

        if format_check and not (colcheck_1 or colcheck_2):
            api_check= get_api_value(c)
            if api_check==True:
                scores.append(100)
            else:
                scores.append(0)
        if not format_check and (colcheck_1 or colcheck_2):
            api_check= get_api_value(c)
            if api_check==True:
                scores.append(100)
            else:
                scores.append(0)
        if not format_check and not (colcheck_1 or colcheck_2):
            scores.append(0)
